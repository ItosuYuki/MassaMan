...
export function DateStrip(props: {
  ...
  const { weekDates, selectedDate, onSelectDate, onPrevWeek, onNextWeek } = props;
  const rangeLabel =
    weekDates.length > 0
  ...
  return (
    <div>
        ...
        {weekDates.map((date) => {
          ...
          return (
            <button
              ...
              className={`flex min-w-11 flex-shrink-0 flex-col items-center gap-1 rounded-xl px-0 py-2 ${
      ...
      <div className="hidden items-center justify-between sm:flex">
        <button onClick={onPrevWeek} aria-label="前の週" className="text-ink-soft">
          ◀
        </button>
